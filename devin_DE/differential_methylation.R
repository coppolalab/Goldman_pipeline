...
rnb.set <- ReadRDSgz("../save/rnb.set.rda")
...
rnb.devin.filter <- !grepl("^[1-2][0-9]$", pheno(rnb.set)$External.Sample.ID)
rnb.devin <- remove.samples(rnb.set, rnb.devin.filter)
devin.annotations <- read.xlsx("./020817 UNGC Web samples sheet Goldman lab.xlsx") %>% filter(!is.na(Sample.or.Library.Name) & !grepl("G19", Sample.or.Library.Name)) 
devin.annotations$Sample.or.Library.Name %<>% str_replace("^.* ", "") %>% str_replace("^.* ", "")
devin.annotations$Cell.Type <- str_c(devin.annotations$Sample.or.Library.Name, devin.annotations$Div, sep = "_")
rnb.devin <- addPheno(rnb.devin, devin.annotations$Cell.Type, "Cell.Type")
rnb.devin <- remove.samples(rnb.devin, is.na(pheno(rnb.devin)$Cell.Type))
devin.annotations.filter <- filter(devin.annotations, !is.na(Div))

rnb.run.qc(rnb.devin, report.dir)
raw.beta <- meth(rnb.devin)
rownames(raw.beta) <- rownames(annotation(rnb.devin, type = "sites"))
write.csv(raw.beta, "./allsites_beta.csv")
biological.clock <- read_csv("./allsites_beta.output.csv")

devin.plot <- data.frame(Cell.Type = pheno(rnb.devin)$Cell.Type, DNAm.Age = biological.clock$DNAmAge)
p <- ggplot(devin.plot, aes(x = Cell.Type, y = DNAm.Age, color = Cell.Type)) + geom_point() + theme_bw()
...
rnb.filter <- rnb.execute.context.removal(rnb.devin)$dataset
...
dred.plot %<>% mutate(Cell.Type = pheno(rnb.norm)$Cell.Type) %>% mutate(Well.Address = devin.annotations.filter$Well.Address)

#Convert this plot ggplot
p <- ggplot(data = dred.plot, aes(x = PC1, y = PC2, col = Cell.Type, label = Well.Address)) + geom_text()
p <- p + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
dred.allsites %<>% mutate(Cell.Type = pheno(rnb.norm)$Cell.Type) %>% mutate(Well.Address = devin.annotations.filter$Well.Address) 
...
dred.genes.plot %<>% mutate(Cell.Type = pheno(rnb.norm)$Cell.Type) %>% mutate(Well.Address = devin.annotations.filter$Well.Address) 

p <- ggplot(data = dred.genes.plot, aes(x = PCA1, y = PCA2, col = Cell.Type, label = Well.Address)) + geom_text()
p <- p + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
p <- p + theme(plot.background = element_blank(), legend.background = element_blank())
